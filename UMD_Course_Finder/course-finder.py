...
# --- Helper: map semester IDs to labels ---
def semester_label(sem_id: str) -> str:
    """Convert semester ID like 202508 -> 'Fall 2025'."""
    sem_id = str(sem_id)
    year, mm = sem_id[:4], sem_id[4:]
    term_map = {"01": "Spring", "05": "Summer", "08": "Fall", "12": "Winter"}
    return f"{term_map.get(mm, 'Unknown')} {year}"

# --- Fetch all valid semesters ---
@st.cache_data(ttl=86400)
def get_valid_semesters():
    try:
        resp = requests.get(f"{BASE_URL}/courses/semesters")
        resp.raise_for_status()
        semesters = [str(s) for s in resp.json()]
        semesters.sort(key=lambda x: int(x), reverse=True)  # newest first
        return semesters
    except Exception:
        return []

valid_semesters = get_valid_semesters()
semester_options = [semester_label(s) for s in valid_semesters]

# Default: Fall 2025 (202508) if available
default_idx = 0
if "202508" in valid_semesters:
    default_idx = valid_semesters.index("202508")

# Sidebar filters
course_or_dept = st.sidebar.text_input(
    "Department or Course ID(s) (e.g., CMSC or CMSC216 or CMSC216,MATH140)",
    "CMSC"
)
professor = st.sidebar.text_input("Professor name (optional)", "")

# ✅ Semester dropdown instead of raw input
semester_idx = st.sidebar.selectbox(
    "Semester",
    range(len(valid_semesters)),
    index=default_idx,
    format_func=lambda i: semester_options[i],
)
